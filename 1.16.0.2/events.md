...
此事件用于开关客户端各种级别的日志。
**Note:** 打开/关闭日志记录级别不仅限于影响广播该事件的脚本，它将影响所有的脚本，比如应用于世界的其他行为包中的脚本。有关日志记录的更多信息，请参见调试部分。
##### 参数
名称 | 类型 | 默认值 | 备注
-|-|-|-
log_errors | 布尔 | 假 | 设置为真以记录客户端上的脚本错误信息
...
此事件用于从的指定玩家的UI栈中移除GUI。事件数据包含要删除的屏幕名称（字符串）。触发事件后，界面将被安排到下一次UI引擎可以这么做的时候被移除。只有通过HTML文件定义的界面可以通过该事件被移除。
